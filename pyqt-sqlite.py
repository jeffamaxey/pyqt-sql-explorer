...
from modify_table_dialog import Ui_Dialog
...
class ModifyTableDialog(QtWidgets.QDialog, Ui_Dialog):
    ...
    def update_code_text(self):
        ...
        for row in range(self.model.rowCount()):
            ...
            if self.tableWidget.cellWidget(row, 4):
                ...
                if self.tableWidget.cellWidget(row, 2).isChecked():
                    field_not = " NOT NULL"

                if self.tableWidget.cellWidget(row, 3).isChecked():
        ...
        if pk_fields:
            sql_lines.append("    PRIMARY KEY(" + ",".join(pk_fields) + ")")
    ...
    def on_commandLinkButton_clicked(self):
        ...
        self.tableWidget.setItem(
            rc, 0, QtWidgets.QTableWidgetItem("Field%d" % self.rc_count)
        )
        self.rc_count += 1
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def on_actionAbout_triggered(self):
        QtWidgets.QMessageBox.about(
            self,
            "About This Software:",
            "This software is written for educational purposes. Not intended for production purposes."
            "\n\n Developer: Jeff Maxey <jeffmaxey@comcast.net>",
        )
    ...
    def on_commandLinkButton_3_clicked(self):
        if self.selectedTable:
            result = QtWidgets.QMessageBox.question(
                ...
                % (self.selectedTable[1], self.selectedTable[0], self.selectedTable[0]),
            )
        if result == QtWidgets.QMessageBox.Yes:
            self.current_database.exec(
                "DROP %s `%s`" % (self.selectedTable[0], self.selectedTable[1])
            )
    ...
    def update_tables_table(self):
        ...
        for typ in ("table", "view"):
            q = self.current_database.exec(
                "SELECT name FROM sqlite_master WHERE type = '%s'" % typ
            )
    ...
    def on_actionNew_triggered(self):
        save_file_dialog = QtWidgets.QFileDialog.getSaveFileName(
            self, "Name of new database", self.work_directory
        )
    ...
    def on_saveQueryAsView_pressed(self):
        ...
        if model:
            if model.rowCount():
                view_name, result = QtWidgets.QInputDialog.getText(
                    self, __appname__, "Enter vieww name:"
                )
                if result:
                    query = self.queryTextEdit.toPlainText()
    ...
    def update_recent_files(self):
        ...
        for i, rc in enumerate(self.recent_files):
            recent_file_action = QtWidgets.QAction(
                "&%d %s" % (i + 1, rc), self, triggered=partial(self.loadDatabase, rc)
            )
